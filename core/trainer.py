...
def train(model, ims, real_input_flag, configs, itr):
    ...
    _, loss_l1,loss_l2_ld,loss_l2_gd,D_loss, gen_d_loss, loss_features_ld,loss_features_gd, d_gt, d_gen, d_gen_pre = model.train(
    ...
    if itr % configs.display_interval == 0:
        ...
        logger.info('itr:{} | L1:{:.8f} | L2:{:.8f}/{:.8f} | D_loss:{:.8f} | Predictor D_loss:{:.8f} | Feature loss:{:.8f}/{:.8f} | D_real: {:.8f} | D_gen: {:.8f} | D_gen_pre: {:.8f}'
                    .format(itr,loss_l1,loss_l2_ld,loss_l2_gd,D_loss,gen_d_loss,loss_features_ld,loss_features_gd,d_gt,d_gen,d_gen_pre))
...
def test(model, test_input_handle, configs, itr):
    if itr > 0:
        ...
        logger = logging.getLogger("stip.test")
        logger.info('Test...')
    ...
    with codecs.open(res_path + '/data.txt', 'w+') as data_write:
        ...
        logger.info('mse per frame: {:.8f}'.format(avg_mse))
        for i in range(configs.total_length - configs.input_length):
            logger.info(str(img_mse[i] / batch_id))
        ...
        logger.info('psnr per frame: {:.8f}'.format(avg_psnr))
        for i in range(configs.total_length - configs.input_length):
            logger.info(str(img_psnr[i] / batch_id))
        ...
        logger.info('ssim per frame: {:.8f}'.format(avg_ssim))
        for i in range(configs.total_length - configs.input_length):
            logger.info(str(ssim[i] / batch_id))
        ...
        logger.info('lpips per frame: {:.8f}'.format(avg_lpips))
        for i in range(configs.total_length - configs.input_length):
            logger.info(str(img_lpips[i] / batch_id))
