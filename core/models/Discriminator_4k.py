...
class Discriminator(nn.Module):
    ...
    def forward(self, input_tensor):
        # print(input_tensor.shape, self.height, self.n)
        output_tensor = []
        output_features = []
        for i in range(input_tensor.shape[1]):
            ...
            # print(input_tensor.shape, features.shape)
...
# D = Discriminator(128, 160, 3, 64).cuda()
# input_tensor = torch.rand((4, 11, 3, 128, 160)).cuda()
# output_tensor, output_features = D(input_tensor)
# print(output_tensor, output_tensor.shape, output_features.shape)
